...
using LPK.ShopEcommerce.MultiTenancy;
...
public class ShopEcommerceAdminHttpApiHostModule : AbpModule
{
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<ShopEcommerceDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}LPK.ShopEcommerce.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopEcommerceDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}LPK.ShopEcommerce.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopEcommerceAdminApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}LPK.ShopEcommerce.Admin.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<ShopEcommerceAdminApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}LPK.ShopEcommerce.Admin.Application"));
            });
        }
    }
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            c.SwaggerEndpoint("/swagger/v1/swagger.json", "ShopEcommerce Admin API");

            var configuration = context.ServiceProvider.GetRequiredService<IConfiguration>();
            c.OAuthClientId(configuration["AuthServer:SwaggerClientId"]);
            c.OAuthScopes("ShopEcommerce");
        });
